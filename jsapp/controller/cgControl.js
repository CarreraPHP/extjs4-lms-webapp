...
Ext.define('MyApp.controller.cgControl', {
	...
	forumView : function(me, e, opts) {
		...
		if(!forumComponent) {
			...
			var grid = forumComponent.down('gridpanel');
	...
	onForumSelect : function(me, record, index, opts){
		//var buttonRef = Ext.ComponentQuery.query('forumthreadview #delete-reply');
	...
	replyForum : function(me, e, opts){
		...
		var parentField = Ext.ComponentQuery.query('#reply-forum-view form textfield[name="description"]');
		parentField[0].setValue(record.description);
	...
	onForumTopicSelectionChange : function(model, selectedModel, opts) {3
