...
Ext.define('MyApp.controller.wsControl', {
	...
	views : ['workspaceView', 'CategoryTreeView', 'CKEditor', 'ContentGridView', 'AddTopicView', 'chartView', 'ContentDisplayView', 'advancesearchView', 'HomeScreenView', 'CourseAccessView', 'ContentFullScreenView', 'ContentForumView', 'AddForumTopicView'],
	init : function() {
		this.control({
			'categorytreeview' : {
	...
	onTopicRightClick : function(view, node, group, e, opts) {
		...
		if(grid.coursePermission.delete ==
		true || permission.delete == true) {
			menu.add({
...
//			var menuTpl = new Ext.XTemplate(['<div class="databox"><ul>', '<li><div>{name}</div></li>', '<li><div>{description}</div></li>', '</ul></div>']);
					var o = Ext.ComponentQuery.query("contentforumview #forumcontent"), pane = o[0];
//					pane.tpl = menuTpl;
	...
	forumView : function(me, e, opts) {
		...
		if(!forumComponent) {
			...
			var grid = forumComponent.down('container').down('gridpanel');
			grid.store.proxy.extraParams.parent = record.data.instanceid;
			grid.store.load();
		}
